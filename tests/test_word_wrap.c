...
// overflowing whitespace hangs on the row it follows, advance_w is the ink
// extent on rows another row follows, separators and budgets are untouched,
// and alignment and the scissor test read the ink extent.
//
    ...
    WLX_Align align, WLX_Text_Line_Record *recs, size_t cap)
{
    ...
    bool ok = wlx_text_prepare_lines_slice(ctx, wlx_rect(0, 0, row_w, 100), text, strlen(text),
        ww_style(), (WLX_Text_Prepare_Opt){ .align = align, .wrap = wrap }, recs, cap, &res);
    test_frame_end(ctx);
