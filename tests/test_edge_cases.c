...
TEST(edge_offsets_zero_total) {
    ...
    float off[4];
    wlx_compute_offsets(off, 3, 0.0f, NULL);
...
TEST(edge_offsets_zero_total_with_sizes) {
    ...
    float off[3];
    wlx_compute_offsets(off, 2, 0.0f, sizes);
    ASSERT_EQ_F(off[0], 0.0f, EDGE_EPS);
...
TEST(edge_offsets_px_exceeds_total) {
    ...
    float off[3];
    wlx_compute_offsets(off, 2, 100.0f, sizes);
    ASSERT_EQ_F(off[0], 0.0f, EDGE_EPS);
...
TEST(edge_offsets_very_large_total) {
    ...
    float off[3];
    wlx_compute_offsets(off, 2, 1000000.0f, NULL);
...
TEST(edge_offsets_many_slots) {
    ...
    float off[17];
    wlx_compute_offsets(off, 16, 1600.0f, NULL);
    ASSERT_EQ_F(off[0], 0.0f, EDGE_EPS);
...
TEST(edge_offsets_min_larger_than_slot) {
    ...
    float off[2];
    wlx_compute_offsets(off, 1, 100.0f, sizes);
    ASSERT_EQ_F(off[0], 0.0f, EDGE_EPS);
...
TEST(edge_offsets_max_smaller_than_total) {
    ...
    float off[2];
    wlx_compute_offsets(off, 1, 1000.0f, sizes);
    ASSERT_EQ_F(off[0], 0.0f, EDGE_EPS);
...
TEST(edge_offsets_min_equals_max) {
    ...
    float off[2];
    wlx_compute_offsets(off, 1, 500.0f, sizes);
    ASSERT_EQ_F(off[0], 0.0f, EDGE_EPS);
    ASSERT_EQ_F(off[1], 100.0f, EDGE_EPS);
}
...
TEST(edge_offsets_mixed_all_types) {
    ...
    float off[5];
    wlx_compute_offsets(off, 4, 400.0f, sizes);
    ASSERT_EQ_F(off[0], 0.0f, EDGE_EPS);
    ASSERT_EQ_F(off[1], 50.0f, EDGE_EPS);
