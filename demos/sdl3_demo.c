...
int main(void) {
    ...
    while (running) {
        ...
        wlx_textbox(ctx, "SDL3 Backend Demo", .height = 52, .font_size = 30, .align = WLX_CENTER, .back_color = WLX_RGBA(28, 32, 45, 255));

        char click_label[64];
        snprintf(click_label, sizeof(click_label), "Clicks: %d", click_count);
        wlx_textbox(ctx, click_label, .height = 40, .font_size = 18, .align = WLX_CENTER, .back_color = WLX_RGBA(20, 24, 34, 255));

        if (wlx_button(ctx, "Click me", .height = 44, .font_size = 20, .align = WLX_CENTER, .back_color = WLX_RGBA(55, 70, 95, 255))) {
            click_count++;
        }

        wlx_checkbox(ctx, "Enable feature", &check_enabled, .height = 40, .font_size = 20, .align = WLX_LEFT);
        ...
        wlx_textbox(ctx, status, .height = 40, .font_size = 18, .align = WLX_LEFT, .back_color = WLX_RGBA(20, 24, 34, 255));
